...
class QuestionGenerator:
    ...
    def askWhoWhat(self, root):
        ...
        for r in rootsSplitCC:
            ...
            if found[0]:
                yield (r.toSentence().lower(),
                    self.escapeNumber(answer[0]))
            else:
                pass
            found[0] = False
            answer[0] = None
    ...
    def askHowMany(self, root):
        ...
        for r in roots:
            ...
            if r.children[0].children[-1].className != '.':
                ...
                r.children[0].children[-1].text = '.'
            if found[0] and \
                    not answer[0] in blackListNumberNoun:
                r.answer = answer[0]
                # self.whMovement(r)
            ...
            if node.className == 'NP':
                for child in node.children:
                    ...
                    if (child.className == 'NN' or
                        ...
                        obj[0] = child
                if question is not None and obj[0] is not None:
                    if obj[0].text != 'screen' and obj[0].text != 'screens':
                        qa[0].append((question,
                                    answer[0].lower()))
                    obj[0] = None
                    question = None
                    answer[0] = None
